 ...
 * project filters the note list. Reuses the sidebar's nav-item / proj-item /
 * nav-group styles so it reads as the same family.
 */
export function OrgPane({
  ...
  return (
    <aside className="notes-org">
      <div className="notes-org-head">Folders</div>
      ...
      >
        ...
        Inbox / Unfiled
        {inboxCount > 0 ? <span className="nav-count">{inboxCount}</span> : null}
      </button>
        ...
        <div key={group.label}>
          ...
          {group.projects.map((p) => (
            <button
              ...
              className={
                "proj-item" +
                (folder.kind === "project" && folder.id === p.id ? " on" : "") +
                (p.paused ? " paused" : "")
              }
              onClick={() =>
