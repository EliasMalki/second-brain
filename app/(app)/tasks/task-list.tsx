 ...
 * cards. The circle completes (optimistic), the rest of the row opens the detail
 * panel. Project color shows as a 3px left edge that thickens on hover.
 */
...
function Row({
  ...
  const cls = ["t-row", edge ? "edged" : "", selected ? "sel" : ""]
    .filter(Boolean)
    .join(" ");
  ...
  return (
    ...
    >
      <button
        type="button"
        className={done ? "t-check done" : "t-check"}
        onClick={(e) => {
          e.stopPropagation();
          onCheck();
        }}
      ...
      <span className={done ? "t-ttl done" : "t-ttl"}>{task.title}</span>
