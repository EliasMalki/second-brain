...
import { isBusinessHoursNow, todayISO } from "../domain/dates";
 ...
 */
export async function listRecentBriefs(
  db: Db,
  orgId: string,
  ownerId: string,
  limit = 30,
...
export async function generatePayload(
  ...
  const [overdue, todays, projects] = await Promise.all([
    listOverdueTasks(db, orgId),
    listTasksScheduledBetween(db, orgId, today, today),
    listProjects(db, orgId, { includeArchived: true }),
  ]);
 ...
 */
export async function getFirstOpenBrief(
  db: Db,
  orgId: string,
  ownerId: string,
  ...
  const { payload, taskIds } = await generatePayload(db, orgId);
  const { error: insErr } = await db.from("briefs_log").insert({
    org_id: orgId,
    owner_id: ownerId,
    kind: "daily" as const,
    generated_for: today,
