...
import { appendToWorkflowNote, getNote } from "./notes";
...
export async function listPendingPrompts(db: Db, orgId: string): Promise<Prompt[]> {
...
export async function getPrompt(db: Db, orgId: string, id: string): Promise<Prompt | null> {
  const { data, error } = await db
    .from("prompts")
    .select("*")
    .eq("org_id", orgId)
...
export async function dismissPrompt(db: Db, orgId: string, id: string): Promise<void> {
 ...
 */
export async function reopenPrompt(db: Db, orgId: string, id: string): Promise<void> {
  const { error } = await db
    .from("prompts")
...
export async function resolveProjectForPrompt(
  ...
  switch (prompt.relates_type) {
    ...
    case "task":
      return (await getTask(db, orgId, prompt.relates_id))?.project_id ?? null;
    case "record":
      return (await getRecord(db, orgId, prompt.relates_id))?.project_id ?? null;
    case "note":
      return (await getNote(db, orgId, prompt.relates_id))?.project_id ?? null;
    default:
      return null;
  }
}
...
export async function answerQuestionPrompt(
  ...
  const prompt = await getPrompt(db, orgId, id);
  if (!prompt) return;

  const projectId = await resolveProjectForPrompt(db, orgId, prompt);
  if (projectId) {
    const today = new Date().toISOString().slice(0, 10);
    const note = await appendToWorkflowNote(db, orgId, ownerId, projectId, {
      date: today,
      question: prompt.text,
      answer: answerText,
    });
    await linkPromptToWorkflowNote(db, orgId, id, note.id);
  }

  await answerPrompt(db, orgId, id, answerText);
}
