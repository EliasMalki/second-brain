...
import { logActivity, type ActivityAction, type ActivityActor } from "./activity";
...
export async function bulkCompleteTasks(db: Db, orgId: string, ids: string[]): Promise<void> {
  for (const id of ids) await completeTask(db, orgId, id);
}
 ...
 */
async function hiddenProjectIds(db: Db, orgId: string): Promise<Set<string>> {
...
export async function listOverdueTasks(db: Db, orgId: string): Promise<Task[]> {
  const { data, error } = await db
    .from("tasks")
    .select("*")
    .eq("org_id", orgId)
 ...
 */
export async function listBacklogTasks(db: Db, orgId: string, limit = 50): Promise<Task[]> {
  const { data, error } = await db
    .from("tasks")
    .select("*")
    .eq("org_id", orgId)
 ...
 */
export async function listUnfiledTasks(db: Db, orgId: string): Promise<Task[]> {
  const { data, error } = await db
    .from("tasks")
    .select("*")
    .eq("org_id", orgId)
 ...
 */
export async function listCompletedTasks(db: Db, orgId: string, limit = 100): Promise<Task[]> {
  const { data, error } = await db
    .from("tasks")
    .select("*")
    .eq("org_id", orgId)
...
export async function getTask(db: Db, orgId: string, id: string): Promise<Task | null> {
...
export async function updateTask(
  ...
  if (input.status === "done" || input.status === "cancelled") {
    await dismissTaskNudges(db, orgId, id);
  }
  return data;
}
...
export async function completeTask(
  ...
  return (await completeTaskInternal(db, orgId, id, actor)).task;
}
 ...
 */
async function dismissTaskNudges(db: Db, orgId: string, taskId: string): Promise<void> {
  const { error } = await db
    .from("prompts")
    .update({
...
export async function completeTaskWithSpawn(
  ...
  return completeTaskInternal(db, orgId, id, actor);
}
...
async function completeTaskInternal(
  ...
  if (data.recurrence_id && data.completed_at) {
    try {
      spawned = await spawnNextCompletionInstance(db, orgId, data);
    } catch (e) {
